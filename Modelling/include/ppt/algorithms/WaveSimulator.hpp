...
template <class ExecSpace> class WaveSimulator
{
  ...
  public:
    ...
    void store_velmodel(const char *filename) const;
    void store_wavefield(const char *filename) const;
    void store_recorded_data(const char *filename) const;
...
/**
 * @brief Store the underlying velocity model in a numpy file.
 *
 */
template <class ExecSpace> void WaveSimulator<ExecSpace>::store_velmodel(const char *filename) const
{
    std::vector<float_type> v(velmodel.get_nElems());
    MemSpace::copyToHost(v.data(), velmodel.get_ptr(), velmodel.get_nElems());
    const long unsigned lshape[] = {static_cast<long unsigned>(_nz), static_cast<long unsigned>(_nx)};
    npy::SaveArrayAsNumpy(filename, false, 2, lshape, v);
}

/**
 * @brief Store the current wavefield in a numpy file.
 *
 */
template <class ExecSpace> void WaveSimulator<ExecSpace>::store_wavefield(const char *filename) const
{
    std::vector<float_type> w(wavefield.get_nElems());
    MemSpace::copyToHost(w.data(), wavefield.get_ptr(), wavefield.get_nElems());
    const long unsigned lshape[] = {static_cast<long unsigned>(_nz), static_cast<long unsigned>(_nx)};
    npy::SaveArrayAsNumpy(filename, false, 2, lshape, w);
}

/**
 * @brief Store the recorded from the receivers data to a numpy file
 *
 */
template <class ExecSpace> void WaveSimulator<ExecSpace>::store_recorded_data(const char *filename) const
{
    const long unsigned lshape[] = {static_cast<long unsigned>(_nt), static_cast<long unsigned>(_nx)};
    npy::SaveArrayAsNumpy(filename, false, 2, lshape, receivers);
}
